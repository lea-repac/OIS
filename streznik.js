...
// Vrne nazive strank (seznam imen in priimkov) glede na ID-je prijavljenih strank
const vrniNaziveStrank = (seznamStrankeId, povratniKlic) => {
    pb.all("SELECT Customer.CustomerId AS id, \
                   Customer.Country AS drzava, \
                   Customer.FirstName || ' ' || Customer.LastName AS naziv \
         FROM   Customer \
         WHERE  Customer.CustomerId IN (" + seznamStrankeId.join(",") + ")",
        (napaka, vrstice) => {
            if (napaka) povratniKlic(""); else povratniKlic(vrstice.length > 0 ? vrstice : "");
        });
};
...
const vrniSeznamDestinacij = (povratniKlic) => {
    pb.all("SELECT   Destination.OsmId AS id, \
         ...
         WHERE    Destination.GeometryId = Geometry.GeometryId AND \
                  ...
                  Coordinate.lat IS NOT NULL AND \
                  Coordinate.lng IS NOT NULL \
    ...
    if (!zahteva.session.kosarica || zahteva.session.kosarica.length == 0) {
        ...
        pb.all("SELECT   Destination.OsmId AS id, \
         ...
         GROUP BY Destination.OsmId", (napaka, vrstice) => {
            console.log(JSON.stringify(vrstice));
            povratniKlic(napaka, vrstice);
        });
    }
};
...
const dogodkiInformativnegaDneFRI = (povratniKlic) => {
    axios.get('https://teaching.lavbic.net/cdn/OIS/DN1/dogodki_informativni_dan_FRI.json')
        .then(function (rezultat) {
            if (rezultat.data)
                povratniKlic(rezultat.data);
            else
                povratniKlic("Podatki o dogodkih informativnega dne FRI niso na voljo!");
        })
...
const strankaIzSeje = (zahteva, povratniKlic) => {
    if (!zahteva.session.trenutneStranke) {
        ...
        pb.all("SELECT * \
             FROM   Customer \
             WHERE  Customer.CustomerId = $id", {$id: zahteva.session.trenutneStranke}, (napaka, vrstice) => {
            if (napaka) povratniKlic(false); else povratniKlic(vrstice[0]);
        });
    }
};
...
const vrniRacune = (povratniKlic) => {
    pb.all("SELECT Customer.FirstName || ' ' || Customer.LastName || \
                  ...
                  Invoice.BillingCity AS mestoRacuna, \
                  Invoice.BillingCountry AS drzavaRacuna, \
                  Customer.FirstName || ' ' || Customer.LastName AS nazivStranke, \
                  Customer.Country AS drzavaStranke, \
                  Customer.Phone AS telefonStranke, \
                  Customer.Email AS emailStranke, \
                  Customer.Fax AS faksStranke, \
...
streznik.get("/", (zahteva, odgovor) => {
    ...
    if (!zahteva.session.trenutneStranke) {
        ...
        vrniSeznamDestinacij((napaka, destinacije) => {
            vrniNazivStranke(zahteva.session.trenutneStranke, (nazivOdgovor) => {
                odgovor.render("seznam", {
                    podnaslov: "Nakupovalni seznam",
                    prijavniGumb: "Odjava",
                    seznamDestinacij: destinacije,
                    nazivStranke: zahteva.session.trenutneStranke ? nazivOdgovor : "gosta",
                });
            });
        });
    }
...
streznik.get("/informativni", (zahteva, odgovor) => {
    ...
    if (!zahteva.session.trenutneStranke) {
        ...
        odgovor.render("informativni", {
            ...
            prijavniGumb: "Odjava",
            seznamStrank: [{podatek: "neznan uporabnik"}],
        });
    }
});
...
function pridobiStatistikoRacunov(racuni) {
    let drzave = {};
    let mesta = {};
    let stranke = {};
    let telefon = 0;
    let faks = 0;
    let email = 0;
    racuni.forEach((racun, i) => {
        if (drzave[racun.drzavaRacuna]) drzave[racun.drzavaRacuna]++;
        else drzave[racun.drzavaRacuna] = 1;

        if (mesta[racun.mestoRacuna]) mesta[racun.mestoRacuna]++;
        else mesta[racun.mestoRacuna] = 1;

        if (stranke[racun.nazivStranke]) stranke[racun.nazivStranke]++;
        else stranke[racun.nazivStranke] = 1;

        if (racun.telefonStranke) telefon++;
        if (racun.faksStranke) faks++;
        if (racun.emailStranke) email++;

    });

    // Poiščemo mesta in države, ki so največkrat zastopana na računih
    let najDrzavaIme = Object.keys(drzave).reduce(function (a, b) {
        return drzave[a] > drzave[b] ? a : b
    });
    let najMestoIme = Object.keys(mesta).reduce(function (a, b) {
        return mesta[a] > mesta[b] ? a : b
    });
    let najStrankaNaziv = Object.keys(stranke).reduce(function (a, b) {
        return stranke[a] > stranke[b] ? a : b
    });

    return {
        vsi: racuni.length,
        razlicne_drzave: Object.keys(drzave).length,
        razlicna_mesta: Object.keys(mesta).length,
        razlicne_stranke: Object.keys(stranke).length,
        naj_drzava_na_racunu: {ime: najDrzavaIme, stevilo: drzave[najDrzavaIme]},
        naj_mesto_na_racunu: {ime: najMestoIme, stevilo: mesta[najMestoIme]},
        naj_stranka_na_racunu: {ime: najStrankaNaziv, stevilo: stranke[najStrankaNaziv]},
        komunikacija: {
            email: email,
            ostali: {
                elektronski: [{telefon: telefon}, {faks: faks}],
                konvencionalni: [{golobi: 0}, {mahanje: 1}, {kričanje: -1}]
            }
        }
    };
}
...
streznik.get("/prijava", (zahteva, odgovor) => {
    vrniStranke((napaka1, stranke) => {
        vrniRacune((napaka2, racuni) => {
            let statistikaRacunov = pridobiStatistikoRacunov(racuni);
            ...
            odgovor.render("prijava", {
                ...
                statistika: statistikaRacunov
            });
        });
    });
});

// Prijava ali odjava strank
streznik.get("/prijavaOdjava/:strankeId", (zahteva, odgovor) => {
    if (zahteva.get("referer").endsWith("/prijava")) {
        let idStrank = zahteva.params.strankeId;

        // Izbira več strank za skupinsko prijavo na spletno mesto informativnega dneva FRI
        if (idStrank.indexOf(",") > -1) {
            let trenutneStrankeStr = idStrank.split(",");
            zahteva.session.trenutneStranke = trenutneStrankeStr.map(element => parseInt(element, 10));
            odgovor.redirect("/informativni");
        }
        // Izbira ene stranke za prijavo na spletno mesto nakupovalnega seznama
        else {
            zahteva.session.trenutneStranke = parseInt(idStrank, 10);
            odgovor.redirect("/");
        }
    } else {
        // Odjava stranke
        delete zahteva.session.trenutneStranke;
